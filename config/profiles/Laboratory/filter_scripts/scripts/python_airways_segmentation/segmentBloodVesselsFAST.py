...
def segmentBloodVesselsFAST(filenameInput, filenameOutputVessels):
    ...
    lungs_filter = fast.LungSegmentation.create(extractBloodVessels=True).connect(importer)
    segmentation_lungs = lungs_filter.runAndGetOutputData(2)
    exporter_vessels = fast.MetaImageExporter.create(filenameOutputVessels).connect(segmentation_lungs)
    exporter_vessels.run()
...
def main(argv):
    ...
    segmentBloodVesselsFAST(input_image_path, output_image_path)
