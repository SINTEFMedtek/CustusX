...
def runTotalSegmentator(filenameInput):
    ...
    if os.path.splitext(filenameInput)[1:] != '.nii.gz':
        ...
        os.system (venv_path + '/TotalSegmentator -i' + filenameInput_nii_gz + ' -o ' + venv_path + '/../../segmentations --task lung_nodules')
    else:
        os.system(venv_path + '/TotalSegmentator -i' + filenameInput + ' -o ' + venv_path + '/../../segmentations --task lung_nodules')
        
def copyOutput(filenameInput):
    venv_path = os.path.dirname(sys.executable)
    data_path =  venv_path + '/../../segmentations/'
    filenameInputNoExt = os.path.splitext(filenameInput)[0]
    if(os.path.isfile(data_path + 'lung_nodules.nii.gz')):
        sitk.WriteImage(sitk.ReadImage(data_path + 'lung_nodules.nii.gz'), filenameInputNoExt + '_nodules.mhd')
