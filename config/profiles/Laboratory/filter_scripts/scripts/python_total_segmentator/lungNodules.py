...
def runTotalSegmentator(filenameInput):
    ...
    process = subprocess.Popen(
        [venv_path + '/TotalSegmentator', '-i', filenameInput,
         '-o', venv_path + '/../../segmentations', '--task', 'lung_nodules',
         '--nr_thr_saving', '1'],
    ...
    # lung_nodules has two prediction phases:
    #   1. Fast rough pass (1/1 iters) used for cropping
    #   2. Main pass (many iters) — this is where we show real progress
    # There are multiple Resampling steps before the main pass; we only
    # start tracking tqdm once predicting_count >= 2.
    predicting_count = 0
