...
def segmentTumors(filenameInput, filenameOutput): 
    ...
    if os.path.splitext(filenameInput)[1:] != '.nii.gz':
        ...
        os.system (venv_path + '/lungtumormask ' + filenameInput_nii_gz + ' ' + filenameOutput_nii_gz + ' --lung-filter --threshold 0.5 --radius 3 --cpu')
        sitk.WriteImage(sitk.ReadImage(filenameOutput_nii_gz), filenameOutput)
    else:
        os.system(venv_path + '/lungtumormask ' + filenameInput + ' ' + filenameOutput + ' --lung-filter --threshold 0.5 --radius 3 --cpu')
