...
def runTotalSegmentator(filenameInput):
    ...
    process = subprocess.Popen(
        [venv_path + '/TotalSegmentator', '-i', filenameInput,
         '-o', venv_path + '/../../segmentations', '--task', 'liver_segments_mr',
         '--nr_thr_saving', '1'],
    ...
    # liver_segments_mr has two prediction phases:
...
def copyOutput(filenameInput):
    ...
    # TotalSegmentator's liver_segments_mr task has no combined whole-liver
    # output of its own, so derive one as the union of the 8 Couinaud segments.
    if segmentImages:
