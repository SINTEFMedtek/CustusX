...
def centerlineExtractionFAST(filenameSegmentedAirwaysVolume, filenameOutputCenterline):
    importer = fast.ImageFileImporter.create(filenameSegmentedAirwaysVolume)
    centerline_extractor = fast.CenterlineExtraction.create().connect(importer)
    centerline = centerline_extractor.runAndGetOutputData()
    exporter_centerline = fast.VTKMeshFileExporter.create(filenameOutputCenterline).connect(centerline)
    exporter_centerline.run()
...
def main(argv):
    ...
    if len(sys.argv) > n_argin_expected:  # command string is sys.argv[0]
        ...
        output_centerline_path = sys.argv[2]  # Second argument should always be destination file
        print(output_centerline_path)
        #centerline_path = sys.argv[3]  # Third argument should always be destination centerline file
    ...
    centerlineExtractionFAST(input_image_path, output_centerline_path)
