...
class CustusXBuilder(object):
    ...
    def _getTestResultsPath(self):
        testRunner = self._getTestRunner()
        return testRunner.generateOutpath(self.assembly.controlData.getCustusXRepositoryPath())
...
#        custusxsetup = self.assembly.getComponent(cxsetup.cxPrivateComponents.CustusXSetup)
    ...
    def _getStandardInstallerPackagePath(self):
        ...
        #Use current project instead of CustusX:
#        if self.assembly.controlData.system_base_name == 'Fraxinus':
        custusxLoc = self.assembly.controlData.getCustusXRepositoryLocation()
        repositoryPath = custusxLoc[0] + '/' + custusxLoc[1] + '/' + self.assembly.controlData.system_base_name
#        else:
#          repositoryPath = self.assembly.controlData.getCustusXRepositoryPath()
        return '%s/package_artefacts' % repositoryPath
