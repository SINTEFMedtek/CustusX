...
public:
  ...
  DICOMModel::IndexType StartLevel;
  DICOMModel::IndexType EndLevel;
};
...
{
  this->RootNode     = 0;
  this->StartLevel = DICOMModel::RootType;
  this->EndLevel = DICOMModel::ImageType;
}
...
/*
//------------------------------------------------------------------------------
QModelIndexList DICOMModelPrivate::indexListFromNode(const Node* node)const
{
  Q_Q(const DICOMModel);
  Q_ASSERT(node);
...
//------------------------------------------------------------------------------
QModelIndexList DICOMModelPrivate::modelIndexList(Node* node)const
{
...
//------------------------------------------------------------------------------
int DICOMModelPrivate::childrenCount(Node* node)const
{
  int count = 0;
  if (node == 0)
    {
...
//------------------------------------------------------------------------------
Node* DICOMModelPrivate::createNode(int row, const QModelIndex& parentValue)const
{
  Node* node = new Node;
  Node* nodeParent = 0;
  if (row == -1)
...
//------------------------------------------------------------------------------
QVariant DICOMModelPrivate::value(const QModelIndex& parentValue, int row, int column) const
{
  Node* node = this->nodeFromIndex(parentValue);
  if (row >= node->RowCount)
    {
    const_cast<DICOMModelPrivate *>(this)->fetch(parentValue, row + 256);
    }
  return this->value(node, row, column);
}

//------------------------------------------------------------------------------
QVariant DICOMModelPrivate::value(Node* parentNode, int row, int column) const
{
...
//------------------------------------------------------------------------------
QString DICOMModelPrivate::generateQuery(const QString& fields, const QString& table, const QString& conditions)const
{
  ...
  logger.debug ( "DICOMModelPrivate::generateQuery: query is: " + res );
  return res;
}

//------------------------------------------------------------------------------
void DICOMModelPrivate::updateQueries(Node* node)const
{
  ...
  switch(node->Type)
    {
    default:
      Q_ASSERT(node->Type == DICOMModel::RootType);
      break;
    case DICOMModel::RootType:
      ...
      logger.debug ( "DICOMModelPrivate::updateQueries for Root: query is: " + query );
      break;
    case DICOMModel::PatientType:
      ...
      logger.debug ( "DICOMModelPrivate::updateQueries for Patient: query is: " + query );
      break;
    case DICOMModel::StudyType:
      ...
      logger.debug ( "DICOMModelPrivate::updateQueries for Study: query is: " + query );
      break;
    case DICOMModel::SeriesType:
      ...
      logger.debug ( "DICOMModelPrivate::updateQueries for Series: query is: " + query );
      break;
    case DICOMModel::ImageType:
      break;
    }
...
//------------------------------------------------------------------------------
void DICOMModelPrivate::fetch(const QModelIndex& indexValue, int limit)
{
  Q_Q(DICOMModel);
...
QVariant DICOMModel::data ( const QModelIndex & dataIndex, int role ) const
{
  ...
  if (dataIndex.row() >= parentNode->RowCount)
    {
    const_cast<DICOMModelPrivate *>(d)->fetch(dataIndex, dataIndex.row());
    }
...
//------------------------------------------------------------------------------
Qt::ItemFlags DICOMModel::flags ( const QModelIndex & modelIndex ) const
{
  Q_D(const DICOMModel);
...
QModelIndex DICOMModel::index ( int row, int column, const QModelIndex & parentIndex ) const
{
  ...
  // TODO: Here it is assumed that DICOMModel::index is called with valid
...
//------------------------------------------------------------------------------
bool DICOMModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
  Q_D(const DICOMModel);
...
//------------------------------------------------------------------------------
bool DICOMModel::setChildData(const QModelIndex &index, const QVariant &value, int role)
{
  Q_D(const DICOMModel);
  if (role != Qt::CheckStateRole)
    {
...
//------------------------------------------------------------------------------
bool DICOMModel::setParentData(const QModelIndex &index, const QVariant &value, int role)
{
  Q_D(const DICOMModel);
...
//------------------------------------------------------------------------------
void DICOMModel::setDatabase(const QSqlDatabase &db)
{
  Q_D(DICOMModel);
...
//------------------------------------------------------------------------------
void DICOMModel::setDatabase(const QSqlDatabase &db,const QMap<QString, QVariant>& parameters)
{
  Q_D(DICOMModel);
...
//------------------------------------------------------------------------------
void DICOMModel::sort(int column, Qt::SortOrder order)
{
  Q_D(DICOMModel);
...
//------------------------------------------------------------------------------
bool DICOMModel::setHeaderData ( int section, Qt::Orientation orientation, const QVariant & value, int role)
{
  Q_D(DICOMModel);
  if (orientation == Qt::Vertical)
    {
