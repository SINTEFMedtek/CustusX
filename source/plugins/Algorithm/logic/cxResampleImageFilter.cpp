...
bool ResampleImageFilter::execute()
{
	...
	ImagePtr oriented = resampleImage(dataManager(), input, refMi);//There is an error with the transfer functions in this image
	...
	ImagePtr cropped = cropImage(dataManager(), oriented);

	QString uid = input->getUid() + "_resample%1";
	QString name = input->getName() + " resample%1";

	ImagePtr resampled = resampleImage(dataManager(), cropped, Vector3D(reference->getBaseVtkImageData()->GetSpacing()), uid, name);
