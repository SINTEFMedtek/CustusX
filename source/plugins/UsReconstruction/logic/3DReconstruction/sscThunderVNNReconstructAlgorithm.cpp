...
DoubleDataAdapterXmlPtr ThunderVNNReconstructAlgorithm::getDistanceOption(QDomElement root)
{
	return  DoubleDataAdapterXml::initialize("Distance (mm)", "",
            "Max distance from frame to voxel when filling output volume. mm.", 1, DoubleRange(0.1, 10, 0.1), 1,
			root);
}
												 ...
												 vtkImageDataPtr outputData, QDomElement settings)
{
	...
	if (!path.exists())
	{
		messageManager()->sendError("Error: Can't find kernels.ocl in any of\n  " + paths.join("  \n"));
		return false;
	}
	...
	messageManager()->sendDebug(QString("Reconstruct data input size: %1 Mb (compare to CL_DEVICE_GLOBAL_MEM_SIZE)").arg(size, 0, 'f', 1));
	messageManager()->sendDebug(QString("Reconstruct data chunk size: %1 Mb (compare to CL_DEVICE_MAX_MEM_ALLOC_SIZE)").arg(size/10, 0, 'f', 1));
	//                                   + qstring_cast(double(size)/1000/1000) + "MB");
	//messageManager()->sendInfo("input dimensions: "
	...
	//messageManager()->sendInfo("ThunderVNNReconstructAlgorithm::reconstruct ***success***");
#endif // SSC_USE_OpenCL
	return success;
}
