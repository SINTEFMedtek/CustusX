...
void CXVBcameraPath::writePositionToFile(Transform3D prMt)
{
	...
	if (positionFile.open(QIODevice::Append))
	{
		...
		stream << prMt(0,0) << " " << prMt(0,1) << " " << prMt(0,2) << " " << prMt(0,3) << Qt::endl;
		stream << prMt(1,0) << " " << prMt(1,1) << " " << prMt(1,2) << " " << prMt(1,3) << Qt::endl;
		stream << prMt(2,0) << " " << prMt(2,1) << " " << prMt(2,2) << " " << prMt(2,3) << Qt::endl;
	}
	...
	if (timestampFile.open(QIODevice::Append))
	{
		...
		stream << mTimeSinceStartRecording.elapsed() << Qt::endl;
	}
