...
ToolManager::ToolManager() :
			...
			mToolGeoFileTag("geo_file"), mToolSensorTag("sensor"), mToolSensorTypeTag("type"), mToolSensorWirelessTag(
					"wireless"), mToolSensorDOFTag("DOF"), mToolSensorPortnumberTag("portnumber"),
			mToolSensorChannelnumberTag("channelnumber"), mToolSensorRomFileTag("rom_file"), mToolCalibrationTag(
					"calibration"), mToolCalibrationFileTag("cal_file"), mPulseGenerator(igstk::PulseGenerator::New()),
...
std::map<std::string, std::string> ToolManager::getToolUidsAndNames() const
{
	...
	{
		uidsAndNames.insert(
				std::pair<std::string, std::string>(((*iter).second)->getUid(), ((*iter).second)->getName()));
		iter++;
	}
	return uidsAndNames;
}
...
ssc::ToolManager::ToolMapPtr ToolManager::configureTools(QList<QDomNodeList>& toolNodeList)
{
	...
	for (int i = 0; i < toolNodeList.size(); i++)
	{
		...
		QDomElement toolSensorWirelessElement = toolSensorElement.firstChildElement(QString(
		...
		QDomElement toolSensorPortnumberElement = toolSensorElement.firstChildElement(QString(
		...
		QDomElement toolSensorChannelnumberElement = toolSensorElement.firstChildElement(QString(
		...
		QDomElement toolSensorRomFileElement = toolSensorElement.firstChildElement(QString(
				mToolSensorRomFileTag.c_str()));
