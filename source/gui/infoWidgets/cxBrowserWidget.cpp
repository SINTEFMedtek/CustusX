...
BrowserItemModel::BrowserItemModel(QObject* parent) : QAbstractItemModel(parent)
{
  ...
  mFilter = StringProperty::initialize("filter",
      "Filter",
...
StringPropertyPtr BrowserItemModel::getFilter()
{
  return mFilter;
}
...
//  std::vector<ViewGroupPtr> groups = viewManager()->getViewGroups();
  ...
  for (int i=0; i<viewService()->groupCount(); ++i)
  {
	  ViewGroupDataPtr group = viewService()->getGroup(i);
//	  group->get
...
//      }
	  ...
	  std::vector<ImagePtr> images = group->getImages(DataViewProperties::createDefault());
...
void BrowserItemModel::buildTree()
{
  ...
  if (showImages)
  {
	...
	std::map<QString, ImagePtr> images = patientService()->getDataOfType<Image>();
	for (std::map<QString, ImagePtr>::iterator iter=images.begin(); iter!=images.end(); ++iter)
    {
