...
IGTLinkUSStatusMessage::Pointer IGTLinkConversion::encode(ProbeDataPtr input)
{
	...
	retval->SetOrigin(input->mOrigin_p.data());
	// 1 = sector, 2 = linear
...
ProbeDataPtr IGTLinkConversion::decode(IGTLinkUSStatusMessage::Pointer probeMessage, IGTLinkImageMessage::Pointer imageMessage, ProbeDataPtr base)
{
	...
	if (probeMessage)
	{
		...
		retval->setSector(
				...
				0);
		retval->mOrigin_p = Vector3D(probeMessage->GetOrigin());
	...
	if (imageMessage)
	{
		...
		retval->mSpacing = Vector3D(spacing[0], spacing[1], spacing[2]);
		retval->mSize = QSize(size[0], size[1]);
		retval->mClipRect_p = DoubleBoundingBox3D(0, retval->mSize.width(), 0, retval->mSize.height(), 0, 0);
	}
